            ...
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <div class='row'>
                        ...
                        @if ($cost>0)
                        <label class="lead col-xs-6 col-md-4">成本：{{$cost}}</label>
                        @else
                        <label class="col-xs-6 col-md-4 label-danger">请设置成本</label>
                        @endif
                        @if (($userinfo->asset_total-$cost)>0)
                        <label class="lead col-xs-6 col-md-4 label-success">赚了：{{round(($userinfo->asset_total-$cost),2)}}</label>
                        @else
                        <label class="lead col-xs-6 col-md-4 label-warning">赚了：{{round(($userinfo->asset_total-$cost),2)}}</label>
                        @endif
                    </div>
                    @if ($userinfo!=null)
                    <div class='row'>
                ...
                <div class="panel-body">
                    ...
                    <div class='row'>
                        ...
                        <span class='col-xs-3 col-md-2 numeric'>{{$userinfo->asset_net}}</span>
                        <span class='col-xs-3 col-md-2'>BTC:</span>
                        <span class='col-xs-3 col-md-2'>{{$userinfo->free_btc}}</span>
                        <span class='col-xs-3 col-md-2'>LTC:</span>
                        <span class='col-xs-3 col-md-2'>{{$userinfo->free_ltc}}</span>
                        <span class='col-xs-3 col-md-2'>总额:</span>
                        <span class='col-xs-3 col-md-2'>{{$userinfo->asset_total}}</span>
                        <span class='col-xs-6 col-md-2 text-warning'>可用:</span>
                        <span class='col-xs-6 col-md-2 text-warning'>{{$userinfo->free_cny}}</span>
                    </div>
                    @endif
                    ...
                    <div class='row'>
                        <span class='col-xs-3 col-md-2'>BTC:</span>
                        <span class='col-xs-3 col-md-2'>{{$btc_ticker->last_price}}</span>
                        <span class='col-xs-3 col-md-2'>前价:</span>
                        <span class='col-xs-3 col-md-2'>{{$set->btc_my_last_price}}</span>
                        <span class='col-xs-3 col-md-2'>波动:</span>
                        <span class='col-xs-3 col-md-2'>{{$set->btc_n_price}}</span>
                        <span class='col-xs-3 col-md-2'>差价:</span>
                        <span class='col-xs-3 col-md-2'>{{round($btc_ticker->last_price-$set->btc_my_last_price,3)}}</span>
                        <span class='col-xs-3 col-md-2 text-success'>上偏差:</span>
                        <span class='col-xs-3 col-md-2 text-success '>{{$set->btc_n_price*$set->uprate}}</span>
                        <span class='col-xs-3 col-md-2 text-danger'>下偏差:</span>
                        <span class='col-xs-3 col-md-2 text-danger '>{{$set->btc_n_price*$set->downrate}}</span>
                    </div>
                    @endif
                    ...
                    <div class='row'>
                        <span class='col-xs-3 col-md-2'>LTC:</span>
                        <span class='col-xs-3 col-md-2'>{{$ltc_ticker->last_price}}</span>
                        <span class='col-xs-3 col-md-2'>前价:</span>
                        <span class='col-xs-3 col-md-2'>{{$set->ltc_my_last_price}}</span>
                        <span class='col-xs-3 col-md-2'>波动:</span>
                        <span class='col-xs-3 col-md-2'>{{$set->ltc_n_price}}</span>
                        <span class='col-xs-3 col-md-2'>差价:</span>
                        <span class='col-xs-3 col-md-2'>{{round($ltc_ticker->last_price-$set->ltc_my_last_price,2)}}</span>
                        <span class='col-xs-6 col-md-2 text-success'>上偏差:</span>
                        <span class='col-xs-6 col-md-2 text-success'>{{$set->ltc_n_price*$set->uprate}}</span>
                        <span class='col-xs-3 col-md-2 text-danger'>下偏差:</span>
                        <span class='col-xs-3 col-md-2 text-danger '>{{$set->ltc_n_price*$set->downrate}}</span>
                    </div>
                    @endif
                    <h4>我的订单</h4>
